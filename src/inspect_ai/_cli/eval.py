...
def eval_options(func: Callable[..., Any]) -> Callable[..., click.Context]:
    ...
    @click.option(
        ...
        is_flag=True,
        default=None,
        help="Enable periodic Docker CRIU checkpointing for crash resumption (Linux only).",
        envvar="INSPECT_EVAL_CHECKPOINT",
    )
    @click.option(
        "--checkpoint-interval-seconds",
        type=float,
        help="Interval in seconds between checkpoint attempts (default: 300).",
        envvar="INSPECT_EVAL_CHECKPOINT_INTERVAL",
    )
    @click.option(
        "--checkpoint-dir",
        type=str,
        help="Directory for storing CRIU checkpoint data.",
        envvar="INSPECT_EVAL_CHECKPOINT_DIR",
    )
    @click.option(
        "--resume-from-checkpoint",
        type=str,
        default=None,
        help="Resume incomplete samples from a failed eval log using CRIU checkpoints.",
        envvar="INSPECT_EVAL_RESUME_FROM_CHECKPOINT",
    )
    @click.option(
        "--no-score",
        type=bool,
